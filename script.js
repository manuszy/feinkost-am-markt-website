...
const drupelet = (x, y, r, color, dark) => `
  <g>
    <circle cx="${x}" cy="${y}" r="${r}" fill="${color}"/>
    <circle cx="${x - r * 0.28}" cy="${y - r * 0.34}" r="${Math.max(2.5, r * 0.22)}" fill="#fff" opacity=".42"/>
    <path d="M${x + r * 0.1} ${y + r * 0.28}c${r * 0.2}-${r * 0.2} ${r * 0.44}-${r * 0.18} ${r * 0.58} .02" stroke="${dark}" stroke-width="2" stroke-linecap="round" opacity=".28" fill="none"/>
  </g>
`;

const raspberryFruit = (a, b, c) => `
  ${juiceSplash(b)}
  ${leafSvg(105, 34, -24, .68)}
  <g transform="translate(132 102)">
    <path d="M-22-45c11-18 32-23 47-8 14 14 13 35 0 48-11 11-31 10-42 0-12-11-15-27-5-40z" fill="${c}" opacity=".18"/>
    <path d="M-29-51c16 10 35 10 53 0" stroke="#5f9b43" stroke-width="7" stroke-linecap="round" opacity=".86" fill="none"/>
    ${[
      [-31, -29, 13, b], [-10, -39, 14, a], [13, -35, 14, b], [32, -19, 13, a],
      [-43, -8, 13, a], [-23, -13, 15, b], [0, -17, 16, a], [23, -10, 15, b], [43, 6, 13, a],
      [-35, 14, 14, b], [-13, 12, 16, a], [12, 16, 15, b], [34, 25, 13, a],
      [-21, 39, 14, b], [3, 41, 15, a], [25, 45, 13, b]
    ].map(([x, y, r, color]) => drupelet(x, y, r, color, c)).join("")}
    <ellipse cx="1" cy="-2" rx="10" ry="15" fill="${c}" opacity=".32"/>
    <path d="M-36 1c14-18 56-21 75-2" stroke="#fff" stroke-width="3" stroke-linecap="round" opacity=".2" fill="none"/>
  </g>
`;
...
const peachFruit = (a, b, c) => `
  ${juiceSplash(b)}
  ${leafSvg(109, 35, -25, .66)}
  <g transform="translate(130 105)">
    <path d="M-58 5c-5-40 25-68 58-56 32-15 66 10 65 51-1 39-31 67-67 71-35-3-61-29-60-66z" fill="${b}"/>
    <path d="M-52 6c-3-31 20-53 47-45 9 3 16 9 21 17-25 13-36 40-32 82-22-8-34-28-36-54z" fill="${a}" opacity=".92"/>
    <path d="M18-43c30 6 47 28 43 58-4 29-25 49-55 55 17-26 21-58 12-113z" fill="#ffd07a" opacity=".62"/>
    <path d="M1-43c18 33 15 78-8 113" stroke="${c}" stroke-width="5" stroke-linecap="round" opacity=".25" fill="none"/>
    <ellipse cx="-28" cy="-22" rx="16" ry="10" fill="#fff" opacity=".29"/>
  </g>
`;

const mirabellePlum = (x, y, scale, rotate, a, b, c) => `
  <g transform="translate(${x} ${y}) rotate(${rotate}) scale(${scale})">
    <ellipse rx="32" ry="43" fill="${b}"/>
    <path d="M-30-1c3-28 20-45 43-43 16 12 24 31 22 53-2 27-20 47-45 49 16-18 20-42 10-59z" fill="${a}" opacity=".54"/>
    <path d="M-24 8c8 18 26 29 47 27-7 13-19 22-35 23-17-3-29-16-33-34 5-7 12-12 21-16z" fill="#e87846" opacity=".28"/>
    <path d="M3-38c13 25 10 61-8 91" stroke="${c}" stroke-width="4" stroke-linecap="round" opacity=".26" fill="none"/>
    <ellipse cx="-12" cy="-22" rx="10" ry="7" fill="#fff" opacity=".35"/>
    <circle cx="14" cy="-11" r="2.2" fill="${c}" opacity=".26"/>
    <circle cx="-18" cy="11" r="1.8" fill="${c}" opacity=".22"/>
  </g>
`;

const mirabelleFruit = (a, b, c) => `
  ${juiceSplash(b)}
  ${leafSvg(106, 35, -23, .68)}
  <g transform="translate(128 104)">
    ${mirabellePlum(-30, 8, 1, -12, a, b, c)}
    ${mirabellePlum(38, -10, .82, 16, a, "#ffe989", c)}
    ${mirabellePlum(45, 45, .55, 7, a, b, c)}
  </g>
`;

const appleOrangeFruit = () => `
  ${citrusSlice("#e2732f", "#f7a531", "#7b271c")}
  <g transform="translate(72 93) scale(.72)">
    ${roundFruit("#c3362d", "#83b64b", "#6c1f18")}
  </g>
`;
...
const walnutHalf = (x, y, scale, rotate, shell, kernel, dark) => `
  <g transform="translate(${x} ${y}) rotate(${rotate}) scale(${scale})">
    <path d="M-47-1c6-36 43-55 78-34 31 18 30 64 0 86-33 23-76 2-84-32-1-7 1-14 6-20z" fill="${shell}"/>
    <path d="M-35 0c7-25 33-38 58-23 23 14 23 46 0 63-25 18-56 2-62-23-1-6 1-12 4-17z" fill="${kernel}"/>
    <path d="M-5-25c-8 16-6 32 5 47 9-15 8-31 0-47z" fill="#f5deb0"/>
    <path d="M-15-17c-12 11-16 28-8 45M13-17c14 10 18 28 8 45M-2-29c4 19 3 39-4 60" stroke="${dark}" stroke-width="4" stroke-linecap="round" opacity=".36" fill="none"/>
    <path d="M-25 11c9-7 18-8 28-3M8 9c11-7 21-7 30 0M-20 31c10-5 18-4 25 2" stroke="${dark}" stroke-width="3" stroke-linecap="round" opacity=".28" fill="none"/>
    <ellipse cx="-25" cy="-20" rx="12" ry="7" fill="#fff" opacity=".24"/>
  </g>
`;

const walnutFruit = (a, b, c) => `
  ${juiceSplash(b)}
  <g transform="translate(165 83) rotate(8) scale(.72)">
    <rect x="-18" y="-66" width="36" height="28" rx="9" fill="${c}" opacity=".82"/>
    <rect x="-33" y="-42" width="66" height="100" rx="23" fill="${a}"/>
    <path d="M-20-36h40v42h-40z" fill="#fff" opacity=".26"/>
    <path d="M-15-60h30" stroke="#fff" stroke-width="5" opacity=".42"/>
    <path d="M-18-32c13 5 31 6 42 0v87c-16 10-37 9-52-1V-29c3-3 7-3 10-3z" fill="${b}" opacity=".66"/>
  </g>
  ${walnutHalf(93, 132, 1, -16, "#7b4d2a", "#f0cf8d", "#6a3b1d")}
  ${walnutHalf(169, 139, .72, 20, "#8a5b34", "#f4dba6", "#70451f")}
  <path d="M184 55c13 21 28 42 12 63-11 15-38 10-42-8-4-17 15-36 30-55z" fill="${b}" opacity=".42"/>
  <circle cx="205" cy="127" r="6" fill="${b}" opacity=".58"/>
`;
...
const mediterraneanHerbSprig = (x, y, rotate, scale, stem = "#477a36", leaf = "#86ad4a") => `
  <g transform="translate(${x} ${y}) rotate(${rotate}) scale(${scale})">
    <path d="M0 78c7-25 11-50 12-77" stroke="${stem}" stroke-width="4" stroke-linecap="round" fill="none"/>
    ${Array.from({ length: 9 }, (_, index) => {
      const yy = 9 + index * 8;
      const side = index % 2 ? -1 : 1;
      const angle = side > 0 ? -36 : 36;
      return `<ellipse cx="${side * 10}" cy="${yy}" rx="5" ry="14" fill="${leaf}" transform="rotate(${angle} ${side * 10} ${yy})"/>`;
    }).join("")}
  </g>
`;

const garlicClove = (x, y, rotate, scale = 1) => `
  <g transform="translate(${x} ${y}) rotate(${rotate}) scale(${scale})">
    <path d="M-18 8c1-24 17-42 35-45 20 21 10 56-16 70-13-2-20-11-19-25z" fill="#f7efd8"/>
    <path d="M-1-33c7 20 5 45-8 64" stroke="#d9caa9" stroke-width="3" stroke-linecap="round" opacity=".72" fill="none"/>
    <path d="M12-23c7 16 3 36-9 50" stroke="#fff" stroke-width="3" stroke-linecap="round" opacity=".7" fill="none"/>
    <path d="M-4-37c8-7 18-8 28-4-6 5-14 8-28 4z" fill="#84a34a"/>
  </g>
`;

const chiliPepper = (x, y, rotate, scale = 1) => `
  <g transform="translate(${x} ${y}) rotate(${rotate}) scale(${scale})">
    <path d="M-39 8c31-20 66-11 78-41-1 44-37 69-75 57-5-2-6-10-3-16z" fill="#c92822"/>
    <path d="M-24 12c23-3 46-17 59-39" stroke="#ff7a4a" stroke-width="5" stroke-linecap="round" opacity=".52" fill="none"/>
    <path d="M31-33c7-8 16-12 28-11" stroke="#5f8c42" stroke-width="5" stroke-linecap="round" fill="none"/>
  </g>
`;

const herbsScene = (a, b, c) => `
  ${bottleScene(a, b, c, `
    ${mediterraneanHerbSprig(69, 56, -28, .9)}
    ${mediterraneanHerbSprig(103, 63, 18, .72, "#5f713b", "#9bb65d")}
    ${mediterraneanHerbSprig(176, 65, 33, .58, "#477a36", "#6f9e4a")}
    ${garlicClove(74, 139, -18, .86)}
    ${chiliPepper(154, 142, -16, .76)}
    <circle cx="188" cy="130" r="6" fill="#d94b28" opacity=".8"/>
    <circle cx="203" cy="143" r="4" fill="#f09a36" opacity=".74"/>
    <circle cx="92" cy="117" r="4" fill="#b02f28" opacity=".68"/>
  `)}
`;
...
const cherryScene = (a, b, c, herbs = false) => {
  const cherryA = "#d7192f";
  const cherryB = "#f04444";
  const cherryDark = "#7a0717";

  return `
    ${juiceSplash(cherryB)}
    <g transform="translate(128 103)">
      <path d="M-30-15c11-30 27-45 53-55M28-19c-3-29 9-45 31-60" stroke="#5f713b" stroke-width="5" stroke-linecap="round" fill="none"/>
      ${berry(-34, 18, 32, cherryA, cherryDark)}
      ${berry(28, 22, 36, cherryB, cherryDark)}
...
const fruitArt = (kind, a, b, c) => {
  const scenes = {
    ...
    walnut: walnutFruit(a, b, c),
    pumpkin: pumpkinScene(a, b, c),
    peach: peachFruit(a, b, c),
    "peach-passion": `${peachFruit("#e67b2f", "#ffd35c", "#81411d")}<g transform="translate(74 94) scale(.62)">${passionFruit("#6f245f", "#ffd84e", "#271026")}</g>`,
    ...
    mirabelle: mirabelleFruit(a, b, c),
